...
See [Configuration file](docs/operations.md#configuration-file) for discovery and precedence, and
[Configuration reference](docs/operations.md#configuration-reference) for the full per-knob table. Concurrency is
...
`lean-host-mcp` 0.11.0 builds on `lean-rs-worker-parent` / `-child` 0.7, which transitively pin `lean-rs` /
`lean-rs-host` 0.7 (0.7.3). It supports the Lean window `4.30.0 ..= 4.34.0-rc2` and is built and tested against the head of that
window, Lean **4.34.0-rc2**.

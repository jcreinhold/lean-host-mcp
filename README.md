...
`lean-host-mcp` 0.12.0 builds on `lean-rs-worker-parent` / `-child` 0.8, which transitively pin `lean-rs` /
`lean-rs-host` 0.8 (0.8.0). It supports the Lean window `4.30.0 ..= 4.34.0-rc2` and is built and tested against the head of that

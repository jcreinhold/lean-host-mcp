//! Bounded module-query cache keyed on path, source hash, and query kind.
//!
//! Position tools no longer cache a whole-file info tree. Each entry is one
//! already-bounded `process_module_query` outcome, so cache hits avoid
//! re-elaboration without reintroducing unbounded intermediate payloads.
